          ...
          <el-collapse-item>
            <template slot="title">
              <div class="order-title1">
                <div style="line-height: 1.2rem;margin: 0.5rem 0;">
                  订单编号<br />{{ item.orderId }}
                </div>
                <div style="line-height: 1.2rem;margin: 0.5rem 0;">
                  订单交期<br />{{ item.orderDeadline }}
                </div>
                <div style="line-height: 1.2rem;margin: 0.5rem 0;">
                  预计交期<br />{{ item.planDeadline }}
                </div>
                <div></div>
              </div>
            </template>
            <div v-for="dela in item.delays" :key="dela.date">
              ...
              <div class="delay-progress">
                <div style="font-size: 18px;">
                  平均生产进度：{{ (item.avgProgress * 100).toFixed(0) }}%
                </div>
                <div style="font-size: 18px;">
                  实际生产进度：{{ (dela.progress * 100).toFixed(0) }}%
                </div>
                <div></div>
                <div></div>
                <div></div>
              ...
              <div v-if="item.requireHuman != 0">
                ...
                <el-table
                  ...
                  style="width: 100%"
                >
                  <el-table-column label="人员信息" width="80">
                  ...
                  <el-table-column label="最大工作时间">
                    <template slot-scope="scope">
                      <span style="margin-left: 10px">{{
                        scope.row.maxWorkTime
                      }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="实际工作时间">
                    <template slot-scope="scope">
                      <span style="margin-left: 10px">{{
                        scope.row.actualWorkTime
                      }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="其它订单占用时间">
                    <template slot-scope="scope" style="display: flex;">
                      <div
                        v-for="occupation in scope.row.occupations"
                        :key="occupation.orderId"
                      >
                        <div>
                          {{ occupation.orderId }}<br />({{ occupation.time }}h)
                        </div>
                      </div>
                    </template>
                  </el-table-column>
                ...
                >
                  ...
                  <el-table-column label="其它订单占用时间">
                    <template slot-scope="scope" style="display: flex;">
                      ...
                      >
                        <div>
                          {{ occupation.orderId }} （{{ occupation.time }}
                          小时）
                        </div>
                      </div>
                    </template>
              ...
              <div v-if="item.requireLine != 0">
                ...
                <el-table
                  ...
                  style="width: 100%"
                >
                  <el-table-column label="产线信息" width="80">
                    <template slot-scope="scope">
                      <span style="margin-left: 10px">{{
                        scope.row.resourceName
                      }}</span>
