<template>
    <div>
        <div class="table-btn">
            <el-button :disabled="switchGantt" @click="changeToGantt" style="float: right">切换回甘特图</el-button>
        </div>
        <div class="table">
...
<script>
    ...
    export default {
        ...
        props: ['queryData','bodyWidth','blocks','blockSize','bias','dateType','date'],
        data() {
            return {
                currentProduct: null,
                transformedData: [],
            ...
            dataTransform(queryData) {
                ...
                for(let product in transformedData) {
                    if(Object.prototype.hasOwnProperty.call(transformedData,product)) {
                        transformedData[product].sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
                        let tmpObj = {};
